...
const port = 9227;
const profile = `/tmp/nowarfy-playlist-hero-mobile-cdp-${Date.now()}`;
...
try {
  ...
  await command('Emulation.setDeviceMetricsOverride', { width: 375, height: 812, deviceScaleFactor: 3, mobile: true });
  await command('Page.navigate', { url: 'http://127.0.0.1:5174/index.html?playlist-hero-mobile-test=1' });
  ...
  await writeFile('/tmp/nowarfy-playlist-hero-mobile.png', Buffer.from(shot.data, 'base64'));
